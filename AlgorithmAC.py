...
with open("/Users/calliesardina/TestAC/Simulation/tests.yaml", 'r') as file:
    ...
    def receive(node, messageSet, dropProbability):
        received = []
            ...
            for node in nodes:
                if(node not in crashedNodes):
                    for messageSet in messages:
                        #if(not(drop(dropProbability))):
                        M[node.i] = receive(node, messageSet, dropProbability)
    ...
    #outputs = simulation(numNodes, 0.6, numFaultyNodes)
    #for i in range(len(outputs)):
    ...
    def run_task_algAC():
        ...
        file = open("algACSimulation_test4.txt", "w")
